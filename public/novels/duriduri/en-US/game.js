var base_url = resPath + '/baseDurandal'
var xmlPathDurandal = xmlPath
var now_galgame_tag = '_2018_Durandal_now_galgame'
var now_scene_tag = '_2018_Durandal_now_scene'
var now_action_tag = '_2018_Durandal_now_action'
var now_galgame = 1
var sceneList = []
var characterData = []
...
var date_url = ''
var xml_files_all_in_this = {}

// For translatable menu/ui/etc resources, switchable between CN static URL and EN.
...
var indexAudioPlayingflag = false
var innerAudioPlayingflag = false
...
function processAction(act, gotoScene, gotoAction, skipKey, loadKey2) {
  ...
  switch (act.nodeName) {
    ...
    case 'goto':
      lastEventNode = 0
      if (!act.getAttribute('clockwise'))
        gotoA(act.getAttribute('goto'), act.getAttribute('change'), skipKey)
      else {
        buttonBarHideFlag = $('.buttonBar').is(':hidden')
        home_btnHideFlag = $('.home_btn').is(':hidden')
        dialogHideFlag = $('.dialog').is(':hidden')
        gotoClockwise(act.getAttribute('goto'))
      }
      break
...
function thanksWords() {
  ...
  //     pv("Du_" + "End_Newest_WX");
  // } else if (GetQueryString("from") == "bh3") {
  //     pv("Du_" + "End_Newest_BH3");
  // } else if (GetQueryString("from") == "ipz") {
  //     pv("Du_" + "End_Newest_IPZ");
  // } else if (GetQueryString("from") == "cd") {
  //     pv("Du_" + "End_Newest_CD");
  // }
  $('#confirm_1')
    .html('<div class="submit-center"></div>')
    .css(
      'background',
...
var timerClockwise
var timerClockwiseIndex
var sceneKeyClockwise
var angleGradientImage = null
var angleGradientCanvas = document.getElementById('canvas')
var angleGradientCtx
var buttonBarHideFlag = false
var home_btnHideFlag = false
var dialogHideFlag = false

function gotoClockwise(sceneKey) {
  sceneKeyClockwise = sceneKey
  dialogAutoplay('stop') //强制终止自动播放
  setListens() //单击暂时无响应
  //console.log("f " + buttonBarHideFlag + "," + home_btnHideFlag + "," + dialogHideFlag);
  $('.buttonBar').hide()
  $('.home_btn').hide()
  $('.remark_btn').hide()
  $('.top_layer').hide()
  $('.choice_list').hide()
  $('.dialog').hide()
  if (!angleGradientImage) {
    angleGradientImage = new Image()
    angleGradientImage.src = base_url + '/resources/ui/angleGradient.png'
    angleGradientImage.onerror = function () {
      angleGradientImage = null
      if (buttonBarHideFlag) $('.buttonBar').show()
      if (home_btnHideFlag) $('.home_btn').show()
      if (dialogHideFlag) $('.dialog').show()
      gotoA(sceneKeyClockwise, null, null)
    }
    angleGradientImage.onload = function () {
      gotoClockwise(sceneKeyClockwise)
    }
    return
  }
  $('.white').removeClass().addClass('white').hide() //容错
  $('.canvas').show()
  $('.canvas-wrapper').css('z-index', '9999')
  angleGradientCtx = angleGradientCanvas.getContext('2d')
  angleGradientCanvas.width = angleGradientCanvas.clientWidth
  angleGradientCanvas.height = angleGradientCanvas.clientHeight
  angleGradientCtx.translate(
    angleGradientCanvas.clientWidth / 2,
    angleGradientCanvas.clientHeight / 2
  )
  angleGradientCtx.rotate((-90 * Math.PI) / 180)
  timerClockwiseIndex = 0
  timerClockwise = setInterval(function () {
    angleGradientCtx.drawImage(angleGradientImage, 0, 0)
    angleGradientCtx.rotate((1 * Math.PI) / 180)
    timerClockwiseIndex++
    if (timerClockwiseIndex > 360) {
      clearInterval(timerClockwise) //结束setInterval的定时循环
      $('.background')
        .css('transition-duration', '0ms')
        .css('transform', 'scale(1,1)')
        .css('transform-origin', 'center')
      $('.white')
        ...
        .fadeIn(0, function () {
          ...
          if (!home_btnHideFlag) $('.home_btn').show()
          if (!dialogHideFlag) $('.dialog').show()
          Action(sceneKeyClockwise, 0)
          //console.log("e " + buttonBarHideFlag + "," + home_btnHideFlag + "," + dialogHideFlag);
          angleGradientCanvas.width = 0
          angleGradientCanvas.height = 0
          $('.canvas').hide()
          $('.canvas-wrapper').css('z-index', '99')
          $('.white').fadeOut(1000)
        })
    }
  }, 5)
...
function startGame(galgameKey, loadKey) {
  ...
  // pv("Du_" + "Start_" + now_galgame);

  // if (GetQueryString("from") == "wx") {
  //     pv("Du_" + "Start_" + now_galgame + "_WX");
  // } else if (GetQueryString("from") == "bh3") {
  //     pv("Du_" + "Start_" + now_galgame + "_BH3");
  // } else if (GetQueryString("from") == "ipz") {
  //     pv("Du_" + "Start_" + now_galgame + "_IPZ");
  // } else if (GetQueryString("from") == "cd") {
  //     pv("Du_" + "Start_" + now_galgame + "_CD");
  // }
  //检测预加载图片是否加载完毕
  var countIndexTimer = 0
  var preLoadImagesTimer = setInterval(function () {
...
function endGame(keyFlag) {
  ...
  if (keyFlag == null) {
    ...
    // pv("Du_" + "End_" + now_galgame);
    // if (GetQueryString("from") == "wx") {
    //     pv("Du_" + "End_" + now_galgame + "_WX");
    // } else if (GetQueryString("from") == "bh3") {
    //     pv("Du_" + "End_" + now_galgame + "_BH3");
    // } else if (GetQueryString("from") == "ipz") {
    //     pv("Du_" + "End_" + now_galgame + "_IPZ");
    // } else if (GetQueryString("from") == "cd") {
    //     pv("Du_" + "End_" + now_galgame + "_CD");
    // }
...
// CUSTOM: requests XML from user-configurable xmlPathDurandal, if it fails, it'll fall back to main VN repo
function get_xml_ajax_async(xmlName, callBack, fileType, xmlFileURL) {
  if (!xmlFileURL) {
    if (fileType) {
      xmlFileURL = xmlPathDurandal + xmlName + '.' + fileType
    } else {
      xmlFileURL = xmlPathDurandal + xmlName + '.xml?sid=' + Math.random()
    }
  }
  $.ajax({
    type: 'GET',
...
function showremark() {
  ...
  for (i = now_action; i < remarkScene.childNodes.length; i++) {
    ...
    if (remarkEvent.nodeName == 'remark') {
      remarkTextBox.html(remarkEvent.innerHTML) //qq浏览器显示不出字
